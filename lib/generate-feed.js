...
const FEED_CONFIG = {
  ...
  feed_url: 'https://nwnn.l484.com/feed.xml',
...
export function generateFeed(links, options = {}) {
  const feed = new RSS({
    ...FEED_CONFIG,
    pubDate: new Date(),
    ...options
  ...
  for (const link of links) {
    ...
    const item = {
      ...
      date: link.createdAt ? new Date(link.createdAt) : new Date(Date.now())
    };

    // Handle images based on domain
